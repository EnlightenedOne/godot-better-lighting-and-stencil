...
float sample_directional_shadow(uint idx, vec3 vertex) {
    ...
    vec3 geo_normal = normal_interp;

	float shadowmask = 1.0;

	if (!bool(directional_lights.data[i].mask & instances.data[instance_index_interp].layer_mask)) {
		return shadow;
	}

	if (directional_lights.data[i].bake_mode == LIGHT_BAKE_STATIC && bool(instances.data[instance_index_interp].flags & INSTANCE_FLAGS_USE_LIGHTMAP)) {
		return shadow; // Statically baked light and object uses lightmap, skip
	}
